...
# Log loss measure (2.62578)
grid_search = GridSearchCV(estimator = BernoulliNB(), param_grid = parameters, scoring = 'neg_log_loss',
                            cv = skf, n_jobs = -1, refit = False, verbose = 2)
grid_search.fit(X_train, y_train)
print(pd.DataFrame(grid_search.cv_results_))

# Accuracy measure
grid_search = GridSearchCV(estimator = BernoulliNB(), param_grid = parameters, scoring = 'accuracy',
                            cv = skf, n_jobs = -1, refit = False, verbose = 2)
grid_search.fit(X_train, y_train)
print(pd.DataFrame(grid_search.cv_results_))
